...
class AdminBenchHousehold(tk.Toplevel):
    ...
    def __init__(self, master = None,switch_frame=None):
        ...
        self.refreshButton = tk.Button(self.canvas, image=self.RefreshButtonImage, command=lambda:print("ButtonClicked"), borderwidth=0, highlightthickness=0)
        ...
        self.columns = (
            "Household_ID", "Applicant_ID","Family Full Name", "Family Age", "Family Civil Status", 
        ...
        for col in self.columns:
            ...
            self.tree.column(col, width=150, minwidth=250, anchor=tk.CENTER, stretch=tk.YES)                
        ...
        self.FetchHouseHoldData()

    def FetchHouseHoldData(self):
        rows = self.database.FetchApplincatDetailsH()
    ...
    def DeleteRow(self):
        ...
        if selected_items:
            ...
            try:
                self.database.delete_household_details(primary_key)
    ...
    def UpdateHouseholdTable(self,item):
        new_values =[self.entries[col].get() for col in self.columns]
        self.tree.item(item, values=new_values)


        self.database.UpdateHouseHoldDetails(
    ...
    def EditButton(self):
        ...
        self.edit_window.geometry("410x320")
        ...
        self.saveButton = tk.Button(self.edit_window, text="Save",state='disabled', command=lambda: self.UpdateHouseholdTable(item))
